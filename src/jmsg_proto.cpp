...
{
    unordered_map<string, size_t>::iterator iter = m_mapNameToIndex.find(name);
...
{
    unordered_map<int, size_t>::iterator iter = m_mapIdToIndex.find(id);
...
bool JMsgProto::decodeJson(const string& typeName, Json::Value& obj, JMsgProtoDecodeJsonCallback callback, void* args)
{
    ...
    for (string& key : obj.getMemberNames()) {
